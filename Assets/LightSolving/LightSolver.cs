            ...
            for (int x = -starsize; x < starsize; x++)
            {
                ...
                for (int y = -starsize; y < starsize; y++)
                {
                    ...
                    if (y < maximum && y > minimum)
                    {
                        ...
                        //render.SetPixel(Mathf.RoundToInt(pixelcoords.x) + x, Mathf.RoundToInt(pixelcoords.y) + y, new Color(1, 1, 1, 1));
                    }
                    else
                    {
    ...
    void EvaluateCollider(Collider curr)
    {
        ...
        if (curr.gameObject.TryGetComponent<StarPositionFromSphericalCoordinates>(out starclass))
        {
            ...
            if (Mathf.Abs(relativepos.x) < 1 && Mathf.Abs(relativepos.y) < 1)
            {
                ...
                //Draw starlines
                if(CreateStars.main.starlookup.ContainsKey(curr.gameObject.name))
                {
                    if (!cachedpositions.ContainsKey(curr.gameObject.name))
                    {
                        List<string> currentlist = (List<string>)CreateStars.main.starlookup[curr.gameObject.name];
                        foreach (string star in currentlist)
                        {
                            if (!cachedpositions.ContainsKey(star))
                            {
                                cachedpositions.Add(star, pixelcoords);
                            }
                        }
                    }
                    else
                    {
                        Vector2 position1 = pixelcoords;
                        Vector2 position2 = cachedpositions[curr.gameObject.name];
